...
) ENGINE = InnoDB AUTO_INCREMENT = 23 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '收入表' ROW_FORMAT = DYNAMIC;
...
INSERT INTO `income` VALUES (22, 7, '8月份工资', 7426.55, '2021-09-10 09:20:00', '银行：苏州银行\n明细：9月10号发放的8月份工资', '2021-09-10 09:22:39', NULL, NULL);
...
) ENGINE = InnoDB AUTO_INCREMENT = 13 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '笔记表' ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of note
-- ----------------------------
INSERT INTO `note` VALUES (1, 'Apache-poi 直接解析本地Excel', 1, NULL, '\n## 一、相关资源\n<a href=\"https://zhuanlan.zhihu.com/p/74922498\" target=\"_blank\">Java 读写 excel</a>\n<a href=\"https://www.b910.cn/tool/1.htm\" target=\"_blank\">省市对照表 2020版</a>\n\n## 二、代码\n1.`pom.xml`\n\n```xml\n<dependency>\n<groupId>org.apache.poi</groupId>\n<artifactId>poi</artifactId>\n<version>4.1.2</version>\n</dependency>\n\n<dependency>\n<groupId>org.apache.poi</groupId>\n<artifactId>poi-ooxml</artifactId>\n<version>4.1.2</version>\n</dependency>\n```\n\n2.`java`代码\n\n```java\npublic void parseProvAndCity() {\n    File city = new File(\"D:\\\\city.xlsx\");\n\n    try {\n        // 构建Excel工作簿\n        Workbook workbook = WorkbookFactory.create(city);\n        // 获取Excel工作表数量\n        int sheets = workbook.getNumberOfSheets();\n        if (sheets < 1) return; // 表示为空\n        // 获取第一张表, 从0开始\n        Sheet sheet = workbook.getSheetAt(0);\n\n        // 用于存储对象\n        List<SysArea> list = new ArrayList<>();\n        // 获取有数据的行数\n        int rowNumbers = sheet.getLastRowNum();\n        // 遍历开始解析\n        for (int i = 1; i < rowNumbers; i++) {\n            Row row = sheet.getRow(i); // 行\n            // 列,第一列从0开始; 此处获取身份证上的编号\n            String areaNum = String.valueOf(Integer.parseInt(row.getCell(0).toString()));\n            // 排除市以下的区县\n            if (!\"00\".equals(areaNum.substring(4, 6))) continue;\n            // 对应省市\n            String areaName = row.getCell(1).toString();\n            String[] split = areaName.split(\",\");\n            areaName = split[split.length - 1];\n            // 封装实体\n            SysArea sysArea = new SysArea();\n            sysArea.setAreaNum(areaNum);\n            sysArea.setAreaName(areaName);\n            list.add(sysArea);\n        }\n        // 遍历存到数据库\n        list.forEach((sysArea -> jobFairMapper.insertSysArea(sysArea)));\n    } catch (IOException e) {\n        e.printStackTrace();\n    }\n}\n```', 0, 0, '2021-09-06 10:28:48', NULL);
INSERT INTO `note` VALUES (2, 'Maven安装三方Jar包到本地仓库', 1, NULL, '```shell\n// 需要配置Maven本地环境变量\nmvn install:install-file \n    -DgroupId=Msc // groupId\n    -DartifactId=Msc // artifactId\n    -Dversion=1.0 // version\n    -Dpackaging=jar // jar\n    -Dfile=Msc.jar  // 文件名\n\n// 示例, 首先定位到 Msc.jar 所在目录, 再执行下面的命令\nmvn install:install-file -DgroupId=Msc -DartifactId=Msc -Dversion=1.0 -Dpackaging=jar -Dfile=Msc.jar\n```', 0, 0, '2021-09-06 10:32:11', NULL);
...
INSERT INTO `note` VALUES (11, 'LeetCode题解，001：两数之和', 10, NULL, '## 两数之和\n\n**题目描述：**\n1. 给定一个整数数组 nums 和一个整数目标值 target，请你在该数组中找出 和为目标值 target  的那两个整数，并返回它们的数组下标。\n2. 你可以假设每种输入只会对应一个答案。但是，数组中同一个元素在答案里不能重复出现。\n3. 你可以按任意顺序返回答案。\n\n**示例 1：**\n> 输入：nums = [2,7,11,15], target = 9\n> 输出：[0,1]\n> 解释：因为 nums[0] + nums[1] == 9 ，返回 [0, 1] 。\n\n**示例 2：**\n> 输入：nums = [3,2,4], target = 6\n> 输出：[1,2]\n> 示例 3：\n> 输入：nums = [3,3], target = 6\n> 输出：[0,1]\n\n**提示：**\n> 2 <= nums.length <= 104\n> -109 <= nums[i] <= 109\n> -109 <= target <= 109\n> 只会存在一个有效答案\n\n**进阶：你可以想出一个时间复杂度小于 O(n2) 的算法吗？**\n> 来源：力扣（LeetCode）\n> 链接：https://leetcode-cn.com/problems/two-sum\n\n**```java``` 题解:**\n\n```java\npublic int[] twoSum(int[] nums, int target) {\n	if (nums != null && nums.length > 0) {\n		for (int i = 0; i < nums.length; i++) {\n			for (int j = i + 1; j < nums.length; j++) {\n				if (nums[i] + nums[j] == target) {\n					return new int[]{i, j};\n				}\n			}\n		}\n	}\n	throw new IllegalArgumentException(\"No two sum solution\");\n}\n```\n', 0, 0, '2021-09-10 13:13:13', NULL);
INSERT INTO `note` VALUES (12, '公司项目访问地址', 9, NULL, '### 一、办公\n**1. 考勤地址**\n```\nhttp://112.4.75.18/cpcm\n```\n\n**2. 公司邮箱**\n```\nhttp://mail.gmwinlead.com/\n```\n\n### 二、项目\n**1. 复旦项目**\n```\n测试地址：http://192.168.16.158:8080/datacenter/\n远程桌面：\n	IP地址: 192.168.16.158\n	登录名称：Administrator\n	登录密码：guomao-12345\n```\n\n**2. 停车收费系统**\n```\n地址：http://192.168.18.187:8080/parkweb\n```\n\n**3. 苏停车**\n```\n地址：http://192.168.18.226:8087/elifeweb/\n```\n\n**4. 市政府2期项目**\n```\n公网地址：http://yunweigmwinlead.cn/access\n测试地址：http://192.168.18.214:8080/access\n远程桌面：\n	IP地址：192.168.18.214\n	登录名称：Administrator\n	登录密码：guomao-12345\n```\n\n**5. 运维项目**\n```\n测试地址 http://192.168.16.158:8080/dcomw/\n```\n\n------------\n', 0, 0, '2021-09-10 16:35:00', NULL);
...
) ENGINE = InnoDB AUTO_INCREMENT = 11 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '笔记分类表' ROW_FORMAT = DYNAMIC;
...
INSERT INTO `note_cat` VALUES (10, '算法笔记', '00010002', 1, 0, '2021-09-10 13:07:37', '', '0001');
...
) ENGINE = InnoDB AUTO_INCREMENT = 21 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '消费表' ROW_FORMAT = DYNAMIC;
...
INSERT INTO `spend` VALUES (20, 4, '京东购物', 561.80, '2021-09-10 12:00:00', '平台：京东商城\n明细：【硬盘】+【...】', '2021-09-10 12:18:37', NULL, NULL);
